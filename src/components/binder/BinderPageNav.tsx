    ...
    const renderPageButton = (item: (typeof pageStats)[0]) => {
        ...
        return (
            <button
                ...
                className={`group relative flex flex-1 min-w-0 flex-col items-center justify-center rounded-lg transition-colors duration-150 outline-none overflow-hidden select-none ${isMobile ? "h-11 px-0.5 pt-1 pb-2" : "h-12 md:h-13 px-1 pt-1.5 pb-2.5"} ${isActive ? "bg-poke-blue/25 text-white shadow-[0_0_12px_var(--theme-primary-glow)] z-10" : isHovered ? "bg-white/[0.08] text-slate-100" : "bg-white/[0.02] text-slate-400 hover:bg-white/[0.05] hover:text-slate-200"}`}
            >
                <span className={`leading-none font-bold transition-colors duration-150 ${isMobile ? "text-xs" : "text-xs md:text-sm lg:text-base"} ${isActive ? "text-poke-blue font-bold" : isHovered ? "text-slate-100" : "text-slate-300"}`}>{item.pageNum}</span>

                <span className={`font-mono leading-none whitespace-nowrap tracking-tight transition-colors duration-150 mt-1 ${isMobile ? "text-[9px]" : "text-[10px] md:text-[11px]"} ${item.isFull ? "text-emerald-400 font-semibold" : isActive ? "text-poke-blue font-medium" : "text-slate-500 group-hover:text-slate-300"}`}>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[2.5px] sm:h-[3px] bg-white/10 overflow-hidden">
                    <div style={{ width: `${item.percentFilled}%` }} className={`h-full transition-all duration-300 ${item.isFull ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]" : item.percentFilled > 0 ? "bg-poke-blue shadow-[0_0_6px_var(--theme-primary-glow)]" : "bg-transparent"}`} />
                </div>
            </button>
        );
    };
